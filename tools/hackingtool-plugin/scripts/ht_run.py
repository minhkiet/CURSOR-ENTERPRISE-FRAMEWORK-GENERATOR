#!/usr/bin/env python3
"""
ht_run.py — Try-first runner for hackingtool tools (UPSTREAM, unmodified).
...
DOCKER_IMAGE_OVERRIDES: dict[str, str] = {
    ...
    "web_attack.Gobuster": "devworks/gobuster",
        ...
        if err_class == "not_installed":
            print(json.dumps(fallback(
                ...
                hint=(
                    ...
                    f"On docker backend, consider --docker-image with the tool pre-installed."
                ),
                diagnostic=result,
            ), indent=2))
            return
